...
public class UniqueValidatorIsValidShould
{
    ...
    {
        var uniqueValidator = new UniqueValidator<TestUser, string>(x => _users!.Find(i => i.Username!.Equals(x.Username))!);
    ...
    {
        var uniqueValidator = new UniqueValidator<TestUser, string>(x => _users!.Find(i => i.Username!.Equals(x.Username))!, x => x.Id);
        var validator = new TestValidator(v => v.RuleFor(x => x.Username).SetValidator(uniqueValidator!));
        var user = new TestUser { Id = id, Username = username };
        var result = validator.Validate(user);
    ...
    {
        var uniqueValidator = new UniqueValidator<TestUser, string>(x => _users!.Find(i => i.Username!.Equals(x.Username))!, x => x.Id, x => x.Email!);
        var validator = new TestValidator(v => v.RuleFor(x => x.Username).SetValidator(uniqueValidator!));
        var user = new TestUser { Id = id, Email = email, Username = username };
        var result = validator.Validate(user);
